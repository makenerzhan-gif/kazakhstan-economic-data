#!/usr/bin/env python3
"""Update pipeline for the World Bank scalar indicators (Pink Sheet monthly). See
update_bns.py for the design rationale; the item-level World Bank datasets (annual
prices, indices, CMO forecasts) live in update_dims.py."""
...
from fetchers import wb as wb_fetchers  # noqa: E402

AGENCY = "wb"
REPO_ROOT = Path(__file__).resolve().parents[1]

FETCHERS = {
    "OIL_PRICE_BRENT": wb_fetchers.fetch_brent_monthly,
}
INDICATOR_IDS = ["OIL_PRICE_BRENT"]
...
def run(run_logger: pipeline_logging.RunLogger) -> None:
    ...
    for indicator_id in INDICATOR_IDS:
        ...
        metadata.DatasetMetadata(
            ...
            unit=ind["unit"],
            currency="USD",
            geography="World (benchmark price)",
            methodology=manifest_info.get("note", "World Bank Commodity Price Data (the Pink Sheet), nominal US dollars"),
